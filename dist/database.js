...
const connectionString = `postgres://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${AWS_RDS_ENDPOINT}:5432/${ENV === 'prod' ? POSTGRES_TEST_DB : POSTGRES_DEV_DB}`;
...
console.log(`Starting database: ENV=[${ENV}] | database = ${ENV === 'prod' ? POSTGRES_TEST_DB : POSTGRES_DEV_DB}  ConnectionString=${connectionString}`);
